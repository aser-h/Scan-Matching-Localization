...
int main(){
	...
	// I tried tweaking on the Lidar values in order to get better results, but the pose error always overshooted, so i used default values

	// lidar_bp.SetAttribute("upper_fov", "10");              //originally 15 "resulting in a higher spatial resolution within the reduced FOV"
    // lidar_bp.SetAttribute("lower_fov", "-15");             //originally -25
    // lidar_bp.SetAttribute("channels", "64");               //originally 32 "higher resolution scan"
    // lidar_bp.SetAttribute("range", "20");                  //originally 30 "higher spatial resolution within the reduced range"
	// lidar_bp.SetAttribute("rotation_frequency", "120");    //originally 60 "higher temporal resolution"
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
            ...
            double filterRes = 0.5; // filter resolution
            vg.setLeafSize(filterRes, filterRes, filterRes); // setting the voxel grid leaf size
            typename pcl::PointCloud<PointT>::Ptr cloudFiltered (new pcl::PointCloud<PointT>); // creating a new point cloud for filtered data
            vg.filter(*cloudFiltered); // output to cloudFiltered

			// TODO: Find pose transform by using ICP or NDT matching
          
          	Eigen::Matrix4d transform = ICP(mapCloud, cloudFiltered, pose, 4); // four iterations 
			...
			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
          	...
          	PointCloudT::Ptr correctedscan(new PointCloudT); // creating a new point cloud for corrected data

			pcl::transformPointCloud(*cloudFiltered, *correctedscan, transform); // transforming the filtered point cloud
